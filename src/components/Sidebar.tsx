...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 flex flex-col w-64 glass border-r border-zinc-200 dark:border-zinc-800 h-screen transition-transform duration-300
      ...
      `}>
        ...
        <div className="p-6 border-b border-zinc-200/50 dark:border-zinc-800/50 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors lg:hidden cursor-pointer"
            title="Close Sidebar"
          >
            <X className="w-5 h-5" />
          </button>
